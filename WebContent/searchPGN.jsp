...
<%-- 
  - Author: Raunak Kumar
  - Description: user searches for PG acording to his required facilities, occupancy, beds ,budget and gender
  --%>
...
<script langauage="javascript">
   ...
   <script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?libraries=places&sensor=false"></script>
	...
	<script type="text/javascript">
		...
		PG p=null;
		//if(!(null == session.getAttribute("prvVal")))
		p=(PG)session.getAttribute("prvVal");
		//else
		//p=new PG(0,"","","","","","false","false","false","false","","","","","","","","");
...
<form action="searchPGMgr.jsp" method="post">
  ...
  for(College cl: alclg){%>
      ...
      <option value="<%=cl.getName() %>" <%if(c.getName().equals(cl.getName())) {%>selected<%} %>><%=cl.getName() %></option>
    ...
    <%} %>   
	  ...
	  <div style="margin-left: 40em;"><p style="font-weight: bold;">College names:</p><input type="text" list="colleges"  />
	   ...
	   <label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="checkbox2">
            <input type="checkbox" id="checkbox2" class="mdl-checkbox__input" value="FOOD" name="fac" <%if(p.getFOOD().equalsIgnoreCase("true")) {%>checked="checked"<%} %>>
            <span class="mdl-checkbox__label">Food</span>
       </label>
	   </td>
	   <td>
	     <label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="checkbox3">
            <input type="checkbox" id="checkbox3" class="mdl-checkbox__input" value="WIFI" name="fac" <%if(p.getWiFi().equalsIgnoreCase("true")) {%>checked="checked"<%} %>>
            <span class="mdl-checkbox__label">WiFi</span>
       </label>
	   </td>
	   <td>
	     <label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="checkbox4">
            <input type="checkbox" id="checkbox4" class="mdl-checkbox__input" value="TV" name="fac" <%if(p.getTV().equalsIgnoreCase("true")) {%>checked="checked"<%} %>>
            <span class="mdl-checkbox__label">TV</span>
       </label>
	   </td>
	   <td>
	     <label class="mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect" for="checkbox5">
            <input type="checkbox" id="checkbox5" class="mdl-checkbox__input" value="AC" name="fac" <%if(p.getAC().equalsIgnoreCase("true")) {%>checked="checked"<%} %>>
	   ...
	   <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label" style="width:130px;">
        <input class="mdl-textfield__input" type="text" id="sample3"  name="nofoc" value="<%=p.getB1()%>">
	   ...
	   <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label" style="width:90px;">
        <input class="mdl-textfield__input" type="text" id="sample4" name="nofbd" value="<%=p.getP1()%>">
	   ...
	   <input id="slider2" class="mdl-slider mdl-js-slider" type="range"  
         min="0" max="10000" value="<%=p.getB2() %>" tabindex="0" 
         oninput="showMessage(this.value)" onchange="showMessage(this.value)"  name="bdgt">
		 </p>
		 <p style="font-weight:bold">Budget: <div id="message" ><%=p.getB2() %></div></p>
		 <p>
		 <label>Gender:</label><br>
          <input type="radio" id="Houseowner" value="Male" name="user_type" <%if(p.getPG_FOR().equalsIgnoreCase("male")) {%>checked="checked"<%} %>><label for="Houseowner" class="light">Male</label><br>
          <input type="radio" id="Student" value="Female" name="user_type" <%if(p.getPG_FOR().equalsIgnoreCase("female")) {%>checked="checked"<%} %>><label for="Student" class="light">Female</label>
